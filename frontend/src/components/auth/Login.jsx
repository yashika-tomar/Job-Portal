...
const Login = () => {
  ...
  const [input, setInput] = useState({//This line uses the useState hook from React to create two variables:
    //input: This is an object that holds the current state of your form data. Initially, it's set to an empty object with properties given
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        navigate("/");
        toast.success(res.data.message);//bottom-right pop up message 
      }
    } catch (error) {
      console.log(error);
    ...
    <div>
      ...
      <div className='flex items-center justify-center max-w-7xl mx-auto'>
        <form onSubmit={submitHandler} className="w-1/2 border border-grey-200 rounded-md p-4 my-10">
          ...
          {
            loading ? <Button className="w-full my-4"><Loader className='mr-2 h-4 w-4 animate-spin' />Please wait</Button> : <Button type="submit" className='w-full my-4'>Login</Button>
          }
